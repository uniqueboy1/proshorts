...
class ViewOtherVideo extends StatefulWidget {
  final Map<String, dynamic> video;
  const ViewOtherVideo({Key? key, required this.video}) : super(key: key);

  @override
  State<ViewOtherVideo> createState() => _ViewOtherVideoState();
}

class _ViewOtherVideoState extends State<ViewOtherVideo> {
  late OtherVideoController otherVideoController =
      Get.put(OtherVideoController());
  ...
  void countViews() async {
    otherVideoController.controller.addListener(() async {
      if (otherVideoController.controller.value.isPlaying) {
        Duration videoLength = otherVideoController.controller.value.duration;
        Duration watchedDuration =
            otherVideoController.controller.value.position;
        if (watchedDuration >= videoLength * 0.5) {
          bool isAlreadyWatched = await isUserAlreadyWatchedVideo();
          if (!isAlreadyWatched) {
            int views = await fetchViewsOfVideo();
            views = views + 1;
            otherVideoController.viewsCount.value = views;
            // updating views on video screen
            await updateViews(views);
            await Get.put(VideoScreenController())
                .updateVideoDetails(currentVideoId, currentVideoUserId);
          } else {
          ...
          child: Container(
            ...
            child: Stack(children: [
              Center(
                child: AspectRatio(
                  aspectRatio:
                      otherVideoController.controller.value.aspectRatio,
                  child: GestureDetector(
                      onTap: () {
                        otherVideoController.showPlayPauseIcon();
                      },
                      child: VideoPlayer(otherVideoController.controller)),
                ),
              ),
              Obx(
                () => !otherVideoController.isVideoPlaying.value
                    ? const Center(
                        child: Icon(
                        Icons.play_arrow,
                        color: white,
...
class ReportWatchLater extends StatelessWidget {
  ...
  final OtherVideoReportReason? selectedReportReson =
      OtherVideoReportReason.notProgramming;
...
class _ReportVideoState extends State<ReportVideo> {
  OtherVideoController otherVideoController = Get.put(OtherVideoController());

  void reportVideo() async {
    String id = widget.video['_id'];
    Map<String, dynamic> reportMessage = {
      "userInformation": MYPROFILE['_id'],
      "message": "${otherVideoController.selectedReportReason.value}"
    };
    await VideoMethods()
      ...
      children: [
        SimpleDialogOption(
            child: Obx(() => RadioListTile(
                ...
                value: OtherVideoReportReason.notProgramming,
                groupValue: otherVideoController.selectedReportReason.value,
                onChanged: (OtherVideoReportReason? value) {
                  otherVideoController.updateReportReason(value!);
                }))),
        SimpleDialogOption(
            child: Obx(() => RadioListTile(
                title: const Text("Hate and Harrasement"),
                value: OtherVideoReportReason.hateHarrasement,
                groupValue: otherVideoController.selectedReportReason.value,
                onChanged: (OtherVideoReportReason? value) {
                  otherVideoController.updateReportReason(value!);
                }))),
        SimpleDialogOption(
            child: Obx(() => RadioListTile(
                ...
                value: OtherVideoReportReason.nuditySexual,
                groupValue: otherVideoController.selectedReportReason.value,
                onChanged: (OtherVideoReportReason? value) {
                  otherVideoController.updateReportReason(value!);
                }))),
        SimpleDialogOption(
            child: Obx(() => RadioListTile(
                ...
                value: OtherVideoReportReason.misinformation,
                groupValue: otherVideoController.selectedReportReason.value,
                onChanged: (OtherVideoReportReason? value) {
                  otherVideoController.updateReportReason(value!);
                }))),
              ...
              child: Stack(
                children: [
                  ...
                  Positioned(
                      ...
                      child: GestureDetector(
                        onTap: () async {
                          await otherVideoController.updateFollowIcon(
                              widget.video['userInformation']['_id']);
                          await Get.put(VideoScreenController())
                              .updateVideoDetails(
                                  currentVideoId, currentVideoUserId);
                        },
                        child: CircleAvatar(
                            radius: 13,
                            backgroundColor: red,
                            child: Obx(() => Icon(
                                  otherVideoController.followIcon.value,
                                  size: 26,
                                  color: white,
                                ))),
                      )),
                      ...
                      child: GestureDetector(
                        onTap: () {
                          Get.to(() => ViewOtherProfile(
                                userId: widget.video['userInformation']['_id'],
                              ));
                        },
                        child: Text(
                          username,
            ...
            Row(
              children: [
                Expanded(
                    child: Obx(
                  () => Text(
                    otherVideoController.videoDescription.value,
                    style: const TextStyle(
                        color: white,
                        fontSize: 20,
                        fontWeight: FontWeight.bold),
                ...
                Obx(() =>
                    otherVideoController.videoDescription.value.length >= 25
                        ? TextButton(
                            onPressed: () {
                              otherVideoController.toogleSeeMore(widget.video);
                            },
                            child: Text(
                              otherVideoController.seeMore.value,
                              style: const TextStyle(
                                  color: Colors.white,
                                  fontWeight: FontWeight.bold),
                            ),
              ...
              () => Column(
                children: [
                  ...
                  Padding(
                    ...
                    child: Text(
                      otherVideoController.viewsCount.value.toString(),
                      style: const TextStyle(
                          color: white,
                          fontSize: 20,
                          fontWeight: FontWeight.bold),
              ...
              () => Column(
                children: [
                  IconButton(
                      onPressed: () async {
                        print("like clicked");
                        await otherVideoController
                            .changeLike(widget.video['_id']);
                        await Get.put(VideoScreenController())
                            .updateVideoDetails(
                                currentVideoId, currentVideoUserId);
                      },
                      icon: Icon(
                        Icons.thumb_up,
                        color: otherVideoController.likeButtonColor,
                        size: 35,
                      )),
                  Padding(
                    padding: const EdgeInsets.only(top: 3),
                    child: Text(
                      otherVideoController.likeCount.value.toString(),
                      style: const TextStyle(
                          color: white,
                          fontSize: 20,
                          fontWeight: FontWeight.bold),
              ...
              () => Column(
                children: [
                  IconButton(
                      ...
                      icon: Icon(
                        Icons.thumb_down,
                        color: otherVideoController.dislikeButtonColor,
                        size: 35,
                      )),
                  Padding(
                    padding: const EdgeInsets.only(top: 3),
                    child: Text(
                      otherVideoController.dislikeCount.value.toString(),
                      style: const TextStyle(
                          color: white,
                          fontSize: 20,
                          fontWeight: FontWeight.bold),
            ...
            Column(
              children: [
                ...
                Padding(
                    ...
                    child: Obx(
                      () => Text(
                        "${otherVideoController.commentsCount.value}",
                        style: const TextStyle(
                            color: white,
                            fontSize: 20,
                            fontWeight: FontWeight.bold),
            ...
            Column(
              children: [
                ...
                Padding(
                  padding: const EdgeInsets.only(top: 3),
                  child: Text(
                    otherVideoController.shareCount.value.toString(),
                    style: const TextStyle(
                        color: white,
                        fontSize: 20,
                        fontWeight: FontWeight.bold),
